...
export function UserStaffNoteForm({ userId }: { userId: string }) {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setBusy(true); setFeedback(null); const response = await fetch(`/api/admin/users/${userId}/notes`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ note }) }); const body = await response.json() as { message?: string }; setBusy(false); setFeedback({ type: response.ok ? "success" : "error", text: body.message ?? "The note could not be saved." }); if (response.ok) { setNote(""); router.refresh(); } }
