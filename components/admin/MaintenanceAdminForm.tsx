  ...
  return (
    <div className="grid gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3 border-y border-slate-200 py-4">
        <div><p className="font-black text-lab-navy">Current status: <span className={settings.maintenance_enabled ? "text-amber-700" : "text-emerald-700"}>{settings.maintenance_enabled ? "Maintenance enabled" : "Site online"}</span></p><p className="text-sm text-slate-500">Last updated {settings.updated_at ? `${adminDateFormatter.format(new Date(settings.updated_at))} UTC` : "not yet"}{lastUpdatedBy ? ` by ${lastUpdatedBy}` : ""}</p></div>
        <div className="flex flex-wrap gap-2"><Link href="/admin/maintenance/preview" className="inline-flex items-center gap-2 rounded-lg border border-slate-200 px-4 py-3 font-black text-lab-navy"><Eye className="h-4 w-4" />Preview</Link><button onClick={() => requestMode(!settings.maintenance_enabled)} disabled={loading} className={`rounded-lg px-4 py-3 font-black text-white ${settings.maintenance_enabled ? "bg-emerald-700" : "bg-amber-700"}`}>{settings.maintenance_enabled ? "Disable maintenance" : "Enable maintenance"}</button></div>
      </div>
      ...
      <section className="grid gap-5 border-b border-slate-200 pb-7">
        <div><h2 className="text-2xl font-black text-lab-navy">Message and status</h2><p className="text-slate-600">Control the public headline, explanation, badge, timing, and overall progress.</p></div>
      ...
      <section className="grid gap-4 border-b border-slate-200 pb-7"><div><h2 className="text-2xl font-black text-lab-navy">Access and display</h2><p className="text-slate-600">These switches determine who can enter and what visitors see.</p></div><div className="grid gap-3 md:grid-cols-2">{([
      ...
      <section className="grid gap-4 border-b border-slate-200 pb-7"><div className="flex items-center justify-between gap-3"><div><h2 className="text-2xl font-black text-lab-navy">Maintenance tasks</h2><p className="text-slate-600">Order and publish the work visitors can follow.</p></div><button onClick={() => setTasks((items) => [...items, { id: crypto.randomUUID(), title: "New task", description: "", status: "waiting", progress_percent: 0, display_order: items.length * 10, visible: true }])} className="inline-flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 font-black text-lab-navy"><Plus className="h-4 w-4" />Add task</button></div><div className="grid gap-3">{tasks.map((task, index) => <div key={task.id} className="grid gap-3 rounded-lg border border-slate-200 p-4"><div className="grid gap-3 md:grid-cols-[1fr_180px]"><input aria-label="Task title" value={task.title} onChange={(e) => updateTask(task.id, { title: e.target.value })} /><select aria-label="Task status" value={task.status} onChange={(e) => updateTask(task.id, { status: e.target.value as MaintenanceTask["status"] })}><option value="waiting">Waiting</option><option value="in_progress">In progress</option><option value="completed">Completed</option><option value="delayed">Delayed</option></select></div><textarea aria-label="Task description" rows={2} value={task.description ?? ""} onChange={(e) => updateTask(task.id, { description: e.target.value })} /><div className="flex flex-wrap items-center gap-3"><input aria-label="Task progress" className="min-w-40 flex-1" type="range" min="0" max="100" value={task.progress_percent ?? 0} onChange={(e) => updateTask(task.id, { progress_percent: clampProgress(Number(e.target.value)) })} /><span className="text-sm font-black">{task.progress_percent ?? 0}%</span><Toggle compact label="Visible" checked={task.visible} onChange={(visible) => updateTask(task.id, { visible })} /><IconButton label="Move task up" disabled={index === 0} onClick={() => moveTask(index, -1)}><ArrowUp /></IconButton><IconButton label="Move task down" disabled={index === tasks.length - 1} onClick={() => moveTask(index, 1)}><ArrowDown /></IconButton><IconButton label="Delete task" onClick={() => setTasks((items) => items.filter((item) => item.id !== task.id))}><Trash2 /></IconButton></div></div>)}</div></section>

      <section className="grid gap-4"><div className="flex items-center justify-between gap-3"><div><h2 className="text-2xl font-black text-lab-navy">Update log</h2><p className="text-slate-600">Publish short progress notes during longer maintenance windows.</p></div><button onClick={() => setUpdates((items) => [{ id: crypto.randomUUID(), title: "Maintenance update", message: "", published_at: new Date().toISOString(), visible: true }, ...items])} className="inline-flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 font-black text-lab-navy"><Plus className="h-4 w-4" />Add update</button></div>{updates.map((update) => <div key={update.id} className="grid gap-3 rounded-lg border border-slate-200 p-4"><input aria-label="Update title" value={update.title} onChange={(e) => updateUpdate(update.id, { title: e.target.value })} /><textarea aria-label="Update message" rows={2} value={update.message} onChange={(e) => updateUpdate(update.id, { message: e.target.value })} /><div className="flex items-center justify-between gap-3"><Toggle compact label="Visible" checked={update.visible} onChange={(visible) => updateUpdate(update.id, { visible })} /><IconButton label="Delete update" onClick={() => setUpdates((items) => items.filter((item) => item.id !== update.id))}><Trash2 /></IconButton></div></div>)}</section>

      <div className="sticky bottom-3 flex justify-end"><button onClick={() => void save()} disabled={loading || !dirty} className="inline-flex items-center gap-2 rounded-lg bg-lab-navy px-5 py-3 font-black text-white shadow-lab disabled:opacity-50"><Save className="h-4 w-4" />{loading ? "Saving..." : "Save changes"}</button></div>

      {confirmEnable ? <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-4" role="dialog" aria-modal="true" aria-labelledby="enable-title" aria-describedby="enable-description"><div className="w-full max-w-lg rounded-lg bg-white p-6 shadow-2xl"><h2 id="enable-title" className="text-2xl font-black text-lab-navy">Enable maintenance mode?</h2><p id="enable-description" className="mt-3 text-slate-600">Public visitors will be redirected. Admin access remains available when bypass is enabled, and login availability follows the current access setting.</p><div className="mt-6 flex justify-end gap-3"><button onClick={() => { setConfirmEnable(false); updateSetting("maintenance_enabled", false); }} className="rounded-lg border border-slate-200 px-4 py-3 font-black text-lab-navy">Cancel</button><button ref={confirmButtonRef} onClick={() => void save(settings)} disabled={loading} className="rounded-lg bg-amber-700 px-4 py-3 font-black text-white">{loading ? "Enabling..." : "Enable maintenance"}</button></div></div></div> : null}
    </div>
  );
}

function Field({ label, children }: { label: string; children: ReactNode }) { return <label className="grid gap-2 text-sm font-extrabold text-slate-700">{label}<span className="[&>input]:w-full [&>input]:rounded-lg [&>input]:border [&>input]:border-slate-200 [&>input]:px-4 [&>input]:py-3 [&>textarea]:w-full [&>textarea]:rounded-lg [&>textarea]:border [&>textarea]:border-slate-200 [&>textarea]:px-4 [&>textarea]:py-3">{children}</span></label>; }
function Toggle({ label, checked, onChange, compact = false }: { label: string; checked: boolean; onChange: (value: boolean) => void; compact?: boolean }) { return <label className={`flex items-center gap-3 font-bold text-slate-700 ${compact ? "text-sm" : "rounded-lg border border-slate-200 p-4"}`}><input type="checkbox" checked={checked} onChange={(e) => onChange(e.target.checked)} className="h-4 w-4" />{label}</label>; }
function IconButton({ label, onClick, disabled, children }: { label: string; onClick: () => void; disabled?: boolean; children: ReactNode }) { return <button aria-label={label} title={label} onClick={onClick} disabled={disabled} className="grid h-9 w-9 place-items-center rounded-lg border border-slate-200 text-lab-navy disabled:opacity-30 [&>svg]:h-4 [&>svg]:w-4">{children}</button>; }
