...
type ProfileFormProps = {
  ...
  role: string;
  profile: { display_name: string | null; username: string | null; avatar_url: string | null; avatar_source: string; bio: string | null; preferred_language: string | null; experience_level: string | null; learning_goal: string | null; biggest_learning_struggle: string | null };
...
export function ProfileForm({ identity, profile, preferences, interests: initialInterests }: ProfileFormProps) {
  ...
  const [interests, setInterests] = useState(() => initialInterests.filter((item) => onboardingGoals.includes(item as any) && item !== profile.learning_goal));
  const [biggestStruggle, setBiggestStruggle] = useState(profile.biggest_learning_struggle ?? "");
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setLoading(true);
    try {
      const response = await fetch("/api/profile", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ displayName, username, bio, preferredLanguage, experienceLevel, learningGoal, interests, biggestStruggle, dailyMinutes, learningFormat }) });
  ...
  return <div className="grid gap-5"><AvatarPicker identity={identity} hasCustomAvatar={profile.avatar_source === "custom"} /><form onSubmit={handleSubmit} className="grid gap-5 surface-panel">
    ...
    <div className="grid gap-4 md:grid-cols-2"><label className="form-label">Experience level<select className="form-control" value={experienceLevel} onChange={(event) => setExperienceLevel(event.target.value)}>{onboardingExperienceLevels.map((value) => <option key={value}>{value}</option>)}</select></label><label className="form-label">What would you like to learn first?<select className="form-control" value={learningGoal} onChange={(event) => { const goal = event.target.value; setLearningGoal(goal); setInterests((items) => items.filter((item) => item !== goal)); }}>{onboardingGoals.map((value) => <option key={value}>{value}</option>)}</select></label><label className="form-label">Starting language<select className="form-control" value={preferredLanguage} onChange={(event) => setPreferredLanguage(event.target.value)}>{onboardingLanguages.map((value) => <option key={value} value={value}>{languageLabels[value]}</option>)}</select></label><label className="form-label">Minutes per day<select className="form-control" value={dailyMinutes} onChange={(event) => setDailyMinutes(Number(event.target.value))}>{dailyMinuteOptions.map((value) => <option key={value} value={value}>{value} minutes</option>)}</select></label><label className="form-label md:col-span-2">Learning style<select className="form-control" value={learningFormat} onChange={(event) => setLearningFormat(event.target.value)}>{learningFormats.map((value) => <option key={value} value={value}>{formatLabels[value]}</option>)}</select></label></div>
    <fieldset className="grid gap-2"><legend className="type-label">Additional interests <span className="font-normal text-subtle">(optional)</span></legend><div className="flex flex-wrap gap-2">{onboardingGoals.filter((goal) => goal !== learningGoal).map((goal) => <button key={goal} type="button" aria-pressed={interests.includes(goal)} className={interests.includes(goal) ? "btn-secondary border border-primary" : "btn-outline"} onClick={() => setInterests((items) => items.includes(goal) ? items.filter((item) => item !== goal) : [...items, goal])}>{goal}</button>)}</div></fieldset>
    <label htmlFor="learning-struggle" className="form-label">Biggest learning struggle <span className="font-normal text-subtle">(optional)</span><textarea id="learning-struggle" value={biggestStruggle} onChange={(event) => setBiggestStruggle(event.target.value)} className="min-h-24 form-control" maxLength={500} placeholder="What tends to slow you down or feel confusing?" /></label>
