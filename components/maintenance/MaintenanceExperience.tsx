...
export function MaintenanceExperience({ state, returnTo, profile, preview = false }: Props) {
  ...
  if (!settings.maintenance_enabled && !preview) {
    return (
      <main data-maintenance-page className="maintenance-page grid min-h-screen place-items-center px-5 py-12">
        <section className="maintenance-online w-full max-w-xl border border-emerald-200 bg-white p-8 text-center shadow-lab">
          ...
          <h1 className="mt-5 text-4xl font-black text-lab-navy">The lab is online.</h1>
          <p className="mt-3 text-slate-600">Maintenance mode is currently off and LearnToCode Lab is ready.</p>
          <Link href={safeReturn} className="mt-6 inline-flex rounded-lg bg-lab-navy px-5 py-3 font-black text-white focus:outline-none focus:ring-4 focus:ring-lab-blue/30">Return to the lab</Link>
        </section>
      </main>
    );
  }
  ...
  return (
    <main data-maintenance-page className="maintenance-page min-h-screen px-4 py-5 sm:px-6 sm:py-8">
      <div className="mx-auto grid min-h-[calc(100vh-2.5rem)] max-w-6xl overflow-hidden border border-slate-200 bg-white shadow-lab lg:grid-cols-[0.78fr_1.22fr]">
        ...
        <section className="maintenance-content flex flex-col p-6 sm:p-9 lg:p-12" aria-labelledby="maintenance-title">
          ...
          <h1 id="maintenance-title" className="mt-6 max-w-2xl text-4xl font-black leading-tight text-lab-navy sm:text-5xl">{settings.maintenance_title}</h1>
          <p className="mt-4 max-w-2xl text-lg leading-8 text-slate-600">{settings.maintenance_message}</p>

          {settings.show_personalized_message && profile?.displayName ? (
            <p className="mt-5 rounded-lg border border-blue-200 bg-blue-50 p-4 font-bold text-lab-navy">
          ...
          {updates.length ? (
            <section className="mt-7" aria-labelledby="updates-title">
              <h2 id="updates-title" className="text-lg font-black text-lab-navy">Latest updates</h2>
              <div className="mt-3 grid gap-3">
                {updates.slice(0, 3).map((update) => (
                  <article key={update.id} className="rounded-lg border border-slate-200 p-4">
                    <div className="flex flex-wrap justify-between gap-2"><h3 className="font-black text-lab-navy">{update.title}</h3><time dateTime={update.published_at} className="text-xs font-bold text-slate-500">{updateDateFormatter.format(new Date(update.published_at))} UTC</time></div>
...
export function MaintenanceHero({ emergency, preview }: { emergency: boolean; preview: boolean }) {
  return (
    <aside className="maintenance-hero relative flex min-h-80 flex-col justify-between overflow-hidden bg-lab-navy p-7 text-white sm:p-10 lg:min-h-full" aria-label="LearnToCode Lab maintenance">
      <div className="relative z-10 flex items-center gap-3 font-black"><Image src="/learntocodelab-logo.png" width={52} height={52} alt="" className="h-[52px] w-[52px] rounded-lg bg-white object-cover" /><span>LearnToCode Lab</span></div>
      <div className="relative z-10 my-10 grid place-items-center">
        <div className="maintenance-logo-wrap grid aspect-square w-[min(72%,260px)] place-items-center rounded-full border border-white/20 bg-white/10 p-7 backdrop-blur"><Image src="/learntocodelab-logo-original.png" width={260} height={260} alt="LearnToCode Lab" className="h-full w-full object-contain" priority /></div>
      </div>
...
export function MaintenanceProgress({ progress }: { progress: number }) {
  return (
    <section className="mt-7" aria-labelledby="progress-title">
      <div className="flex items-center justify-between gap-3"><h2 id="progress-title" className="text-lg font-black text-lab-navy">Upgrade progress</h2><span className="font-black text-lab-blue">{progress}%</span></div>
      <div className="mt-3 h-3 overflow-hidden rounded-full bg-slate-100" role="progressbar" aria-label="Overall maintenance progress" aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress}><div className="h-full rounded-full bg-gradient-to-r from-lab-teal to-lab-blue transition-[width] duration-500" style={{ width: `${progress}%` }} /></div>
    </section>
  );
}
...
export function MaintenanceCountdown({ remaining }: { remaining: number | null }) {
  ...
  if (remaining === 0) return <p className="mt-6 rounded-lg bg-slate-100 p-4 font-black text-lab-navy" role="status">Checking site status...</p>;
  ...
  return <section className="mt-7" aria-labelledby="countdown-title"><h2 id="countdown-title" className="text-lg font-black text-lab-navy">Estimated return</h2><div className="mt-3 grid grid-cols-4 gap-2" role="timer" aria-live="off">{values.map((item) => <div key={item.label} className="rounded-lg bg-slate-100 p-3 text-center"><strong className="block text-2xl text-lab-navy">{String(item.value).padStart(2, "0")}</strong><span className="text-[11px] font-bold uppercase text-slate-500">{item.label}</span></div>)}</div></section>;
}

export function MaintenanceTaskList({ tasks }: { tasks: MaintenanceTask[] }) {
  return <section className="mt-7" aria-labelledby="tasks-title"><h2 id="tasks-title" className="text-lg font-black text-lab-navy">What we&apos;re working on</h2><div className="mt-3 grid gap-3">{tasks.map((task) => { const meta = taskMeta[task.status]; const Icon = meta.icon; return <article key={task.id} className="rounded-lg border border-slate-200 p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="font-black text-lab-navy">{task.title}</h3>{task.description ? <p className="mt-1 text-sm text-slate-600">{task.description}</p> : null}</div><span className={`inline-flex shrink-0 items-center gap-1 rounded-full px-2.5 py-1 text-xs font-black ${meta.className}`}><Icon className={`h-3.5 w-3.5 ${task.status === "in_progress" ? "maintenance-spin" : ""}`} aria-hidden="true" />{meta.label}</span></div>{task.progress_percent !== null ? <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-slate-100"><div className="h-full bg-lab-blue" style={{ width: `${task.progress_percent}%` }} /></div> : null}</article>; })}</div></section>;
}

export function MaintenanceFooter({ support, email }: { support: string | null; email: string | null }) {
  return <footer className="mt-auto pt-9 text-sm text-slate-500"><p>{support ?? "Thank you for your patience."}</p>{email ? <a className="mt-2 inline-flex font-black text-lab-blue underline-offset-4 hover:underline focus:outline-none focus:ring-4 focus:ring-lab-blue/30" href={`mailto:${email}`}>Contact support</a> : null}<p className="mt-4">&copy; 2026 LearnToCode Lab. All rights reserved.</p></footer>;
}
