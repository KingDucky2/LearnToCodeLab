...
export default async function AdminUserDetail({ params }: { params: Promise<{ userId: string }> }) {
  ...
  const [expandedProfileResult, authResult, attempts, progress, skills, tickets, history, userNotes, languages, recentAttempts] = await Promise.all([
    db.from("profiles").select("id,email,display_name,username,avatar_url,role,onboarding_completed,account_status,account_status_reason,account_status_changed_at,preferred_language,experience_level,learning_goal,bio,biggest_learning_struggle,created_at").eq("id", userId).maybeSingle(),
  ...
  const profileResult = expandedProfileResult.error
    ? await db.from("profiles").select("id,email,display_name,username,avatar_url,role,onboarding_completed,account_status,account_status_reason,account_status_changed_at,preferred_language,experience_level,learning_goal,bio,created_at").eq("id", userId).maybeSingle()
    : expandedProfileResult;
  ...
  const [{ data: actors = [] }, { data: goals = [] }] = await Promise.all([
    actorIds.length ? db.from("profiles").select("id,display_name,email").in("id", actorIds) : Promise.resolve({ data: [] }),
    db.from("learning_goals").select("goal").eq("user_id", userId),
  ]);
  ...
  return <>
    ...
    <div className="grid gap-6 xl:grid-cols-[1fr_380px]">
      <div className="grid gap-6">
        <AdminCard><div className="flex items-center gap-4"><AccountAvatar identity={identity} size="lg" decorative /><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="type-section">{identity.label}</h2><RoleBadge role={profile.role} /></div><p className="truncate text-muted">{identity.email}</p><code className="text-xs text-subtle">{userId}</code></div></div><dl className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3"><IdentityField label="Display name" value={profile.display_name || "Not set"} /><IdentityField label="Username" value={profile.username || "Not set"} /><IdentityField label="Email" value={profile.email || authUser.email || "Not set"} /><IdentityField label="Role" value={profile.role} /><IdentityField label="Account status" value={profile.account_status} /><IdentityField label="Authentication providers" value={providers} /><IdentityField label="Email confirmed" value={authUser.email_confirmed_at ? "Yes" : "No"} /><IdentityField label="Joined" value={<LocalTime value={profile.created_at} />} /><IdentityField label="Last sign-in" value={authUser.last_sign_in_at ? <LocalTime value={authUser.last_sign_in_at} /> : "Never"} /></dl></AdminCard>
      ...
      <aside className="grid content-start gap-6">
        <AdminCard><h2 className="type-card">Learning profile</h2><dl className="mt-4 grid gap-3 text-sm"><IdentityField label="Onboarding" value={profile.onboarding_completed ? "Complete" : "Not complete"} /><IdentityField label="Preferred language" value={profile.preferred_language || "Not set"} /><IdentityField label="Experience" value={profile.experience_level || "Not set"} /><IdentityField label="Learning goal" value={profile.learning_goal || "Not set"} /><IdentityField label="Additional interests" value={goals.map((item: any) => item.goal).filter((goal: string) => goal !== profile.learning_goal).join(", ") || "Not set"} /><IdentityField label="Biggest struggle" value={profile.biggest_learning_struggle || "Not set"} /><IdentityField label="Bio" value={profile.bio || "Not set"} /></dl><p className="mt-4 text-sm text-muted">{languages.data?.length ? languages.data.map((item: any) => `${item.language_slug} (${item.experience_level})`).join(", ") : "No selected language experience."}</p>{recentAttempts.data?.length ? <ul className="mt-4 grid gap-2 text-sm">{recentAttempts.data.map((attempt: any) => <li key={attempt.id} className="rounded-lg bg-surface-secondary p-3"><strong>{attempt.attempt_type}</strong><span className="block text-muted">{attempt.language_slug || "General"} · {attempt.score ?? "No score"} · <LocalTime value={attempt.started_at} /></span></li>)}</ul> : <p className="mt-3 text-sm text-muted">No recent attempts.</p>}</AdminCard>
