...
export default async function AdminUsersPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  let query = db.from("profiles").select("id,email,display_name,username,avatar_url,role,onboarding_completed,account_status,auth_providers,created_at", { count: "exact" });
  if (search) {
    const uuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(search);
    query = query.or(`email.ilike.%${search}%,display_name.ilike.%${search}%,username.ilike.%${search}%${uuid ? `,id.eq.${search}` : ""}`);
  }
