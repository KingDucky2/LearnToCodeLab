...
export default async function AdminPage() {
  ...
  const [{ data: settings }, { data: tasks }, { data: updates }, users, openTickets, suspendedUsers, { data: recentActivity }] = await Promise.all([
  ...
  return (
    <>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-3">
        ...
        <AdminCard><div className="flex items-center justify-between gap-3"><h2 className="type-card">Recent admin activity</h2><Link className="text-sm font-black text-primary" href="/admin/activity">View all</Link></div>{recentActivity?.length ? <div className="mt-4 grid gap-3">{recentActivity.map((event: { id: string; action: string; summary: string; result: string; created_at: string }) => <article key={event.id} className="rounded-lg border border-border p-3"><div className="flex justify-between gap-2"><h3 className="font-black text-foreground">{event.action}</h3><span className="text-xs font-bold uppercase text-subtle">{event.result}</span></div><p className="mt-1 text-sm text-muted">{event.summary}</p><time className="mt-1 block text-xs text-subtle">{dateFormatter.format(new Date(event.created_at))}</time></article>)}</div> : <p className="mt-4 text-sm text-muted">No sensitive admin actions have been recorded yet.</p>}</AdminCard>
      </div>
